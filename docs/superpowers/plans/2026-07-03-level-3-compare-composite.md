...
export function generateComposite(): CompositeQuestion {
  ...
  const d = round2(P - Pp)
  const k = round2(((currentA / currentB - baseA / baseB) / (baseA / baseB)) * 100)
  ...
  const r4 = denom4 !== 0 ? round2(((currentA - currentB) / denom4 - 1) * 100) : 0
...
<template>
  <div class="composite-session">
    ...
    <!-- 已提交反馈 -->
    <div v-if="submitted" class="feedback">正确 {{ correctCount }}/9</div>
